...
def harm_amp(inputarr, freq1, freq2, fsampling): 
    ...
    n2 = ceil(N*f2/fs)
    yenhanced = inputarr
    yenhanced[n1:n2+1] = 2*yenhanced[n1:n2+1]
...
better_audio = np.fft.ifft(audio_fftdat)*N*2**15   #this produced a replica of original file, the file was named newbie
wavfile.write("improved.wav",fs, better_audio.astype(np.int16)) #.astype(np.int16)
...
plt.title('Signal in Time Domain: %s' %letter)
...
plt.ylabel('Amplitude')
plt.title('FFT spectrum Linear Scale: %s' %letter)
plt.legend() # Shows the plot legend
plt.grid(which='major', color='black', linewidth=0.8)
plt.grid(which='minor', color='black', linewidth=0.5)
plt.minorticks_on()
...
plt.title('FFT spectrum Log Scale: %s' %letter)
plt.legend() # Shows the plot legend
plt.grid(which='major', color='black', linewidth=0.8)
plt.grid(which='minor', color='black', linewidth=0.5)
plt.minorticks_on()
